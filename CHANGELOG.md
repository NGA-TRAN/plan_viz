...
- Element `index` keys stay 4 characters (`c000`…) so Excalidraw does not throw `invalid order key: c0g10` on large plans

## [0.1.21] - 2026-09-15

### Fixed
- Node box, title, in-box details, hash tables, and DynamicFilter share an Excalidraw group so they drag together; arrows stay bound and stretch; column labels on those arrows join the child node's group
...
- DataFusion `subquery.slt` (`subqueries=2` and nested `ScalarSubqueryExec`) and `window.slt` (`StreamingTableExec` + stacked `BoundedWindowAggExec`) fixtures

### Fixed
- `ScalarSubqueryExec` children are packed by full subtree width so wide inputs (e.g. TPC-H Q11 HashJoins) do not overlap
- EXPLAIN table parser no longer stops at continuation lines whose expressions contain `+`
...
### Changed
- Simplified npm publish command in CI/CD workflow
- Removed `packages: write` permission from workflow (not required for npm publishing)
- Removed `--provenance` flag from npm publish command

## [0.1.12] - 2025-12-26

### Changed
- Updated GitHub Actions workflow permissions for npm publishing
- Added explicit permissions (contents: read, packages: write, id-token: write) to workflow for better security and reliability
